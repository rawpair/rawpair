...
defmodule RawPairWeb.CoreComponents do
  ...
  use Gettext, backend: RawPairWeb.Gettext
  ...
  def modal(assigns) do
    ...
    >
      <div id={"#{@id}-bg"} class="bg-modal-background fixed inset-0 transition-opacity" aria-hidden="true" />
      <div
        ...
        tabindex="0"
      >
        <div class="flex min-h-full items-center justify-center bg-background">
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-2 block w-full rounded-md border border-white shadow-sm focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
          ...
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-border">
            ...
            >
              <div class="block py-4 pr-6">
                <span class="absolute -inset-y-px right-0 -left-4 group-hover:bg-border sm:rounded-l-xl" />
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                <span class="absolute -inset-y-px -right-4 left-0 group-hover:bg-border sm:rounded-r-xl" />
                <span
                  :for={action <- @action}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(RawPairWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(RawPairWeb.Gettext, "errors", msg, opts)
    end
  end
